...
class SpiderDataset(torch.utils.data.Dataset): 
    ...
    class Metrics:
        ...
        def add_all(self, idx, item, inferred_codes):
            self.results.append((idx, [self.evaluator.evaluate_one(
                item.schema.db_id, item.orig['query'], inferred_code) for inferred_code in inferred_codes]))
